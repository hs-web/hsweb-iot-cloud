            ...
            if (resp.status === 200) {
                $(resp.result).each(function () {
                    if (id != this.id) {
                        ...
                        permissionData.push(this);
                    }
                    permissionDataMap[this.id] = this;
                })
            }
        });
        mini.get('parent-permission-grid').getColumn("permission").renderer = function (e) {
            var permission = permissionDataMap[e.value];

            return permission ? permission.id + "(" + permission.name + ")" : e.value;
        }
