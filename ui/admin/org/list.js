...
require(["authorize"], function (authorize) {
    ...
    importMiniui(function () {
        ...
        require(["miniui-tools", "request"], function (tools, request) {
            ...
            grid.loading("加载中...")
            grid.getColumn("districtId").renderer = function (e) {
                return districtMap[e.value] ? districtMap[e.value].name : "";
            }
            request.createQuery("district/all").exec(function (response) {
                if (response.status === 200) {
                    $(response.result).each(function () {
                        districtMap[this.id] = this;
                    });
                    districtData = tools.listToTree(mini.clone(response.result));
                    // mini.get("district").loadList(districtData);
                }
                search()
            });
